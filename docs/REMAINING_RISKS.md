...
| RR-007 | The repo has an initial local commit, but no GitHub remote, pushed tag, release, or BRAT installation has been created. | GitHub release and BRAT installation cannot be treated as reproducible until the repo is pushed and released. | Create GitHub remote, push, tag a release matching `manifest.json.version`, and verify BRAT install. |
